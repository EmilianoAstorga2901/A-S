import { useState } from 'react';
import { Header } from './components/Header';
import { BalanceCard } from './components/BalanceCard';
import { ActionGrid } from './components/ActionGrid';
import { InvestmentCard } from './components/InvestmentCard';
import { PromotionCard } from './components/PromotionCard';
import { BottomNav } from './components/BottomNav';
import { InvestmentExperience } from './components/InvestmentExperience';
import { quickActions, services, user } from './data/mockData';

function SectionTitle({children,action}){return <div className="section-title"><h2>{children}</h2>{action&&<button>{action}</button>}</div>}
export default function App(){
 const [active,setActive]=useState('Inicio'); const [toast,setToast]=useState('');
 if (active === 'Invertir') return <div className="app-shell investment-shell"><InvestmentExperience onClose={()=>setActive('Inicio')}/></div>;
 const inform=(message)=>{setToast(message);window.clearTimeout(inform.timer);inform.timer=window.setTimeout(()=>setToast(''),2200)};
 return <div className="app-shell">
  <main className="phone-content">
   <div className="page-top"><Header user={user} onNotify={()=>inform('No tenés notificaciones nuevas')}/><BalanceCard balance={user.balance} onDeposit={()=>inform('Ingresar dinero')} onActivity={()=>setActive('Actividad')}/></div>
   <SectionTitle>Acciones rápidas</SectionTitle><ActionGrid items={quickActions} onSelect={inform}/>
   <SectionTitle action="Ver todos">Servicios</SectionTitle><ActionGrid items={services} variant="services" onSelect={inform}/>
   <InvestmentCard user={user} onOpen={()=>setActive('Invertir')}/>
   <SectionTitle action="Ver todos">Beneficios para vos</SectionTitle><PromotionCard/>
   <p className="legal-note">Los rendimientos son simulados y no garantizan resultados futuros.</p>
  </main>
  <BottomNav active={active} onChange={(name)=>{setActive(name); if(name!=='Inicio' && name!=='Invertir') inform(`${name}: próximamente`)}}/>
  {toast&&<div className="toast" role="status">{toast}</div>}
 </div>
}
